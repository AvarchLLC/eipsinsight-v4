...
import { relabelMislabeledPrs } from '@/lib/proposal-ref';
 ...
 *
 * Every rendered string passes through relabelMislabeledPrs, because the EF's
 * verbatim summaries sometimes call a GitHub PR an EIP ("EIP-11905"); we show the
 * corrected "PR-11905" so the summary doesn't send readers chasing a nonexistent
 * proposal.
 */
...
function itemToText(item: unknown): string {
  if (typeof item === 'string') return relabelMislabeledPrs(item);
  if (item && typeof item === 'object') {
    const record = item as Record<string, unknown>;
    for (const key of ['text', 'title', 'topic', 'summary', 'description', 'item']) {
      if (typeof record[key] === 'string') return relabelMislabeledPrs(record[key] as string);
    }
    return relabelMislabeledPrs(
