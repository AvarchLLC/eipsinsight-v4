...
export function SearchBar() {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      setLoading(true);
      try {
        const [proposals, authors, prs] = await Promise.all([
  ...
  return (
    <div className="relative w-full max-w-3xl mx-auto">
      <div className="relative">
        {/* Search Icon */}
        <Search className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-cyan-400/80 pointer-events-none" />
        ...
        <input
          ...
          className={cn(
            ...
            "rounded-full border border-cyan-300/30 bg-black/40",
      ...
      {showDropdown && (query.trim() || hasResults) && (
        <div
          ref={dropdownRef}
          className="absolute mt-3 w-full bg-slate-950/95 border border-cyan-400/20 rounded-xl shadow-2xl z-50 overflow-hidden backdrop-blur-xl"
        ...
        >
          {loading ? (
            ...
            <div className="p-8 text-center">
              <Search className="h-12 w-12 text-slate-500/50 mx-auto mb-3" />
            ...
            <div className="max-h-[480px] overflow-y-auto">
              ...
              {results.proposals.length > 0 && (
                <div className="border-b border-cyan-400/10">
                  <div className="sticky top-0 z-10 px-4 py-2.5 bg-gradient-to-r from-cyan-500/10 to-emerald-500/10 border-b border-cyan-400/20 backdrop-blur-sm">
                  ...
                  {results.proposals.map((result, index) => {
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          "cursor-pointer p-4 transition-all border-b border-slate-800/50 last:border-0",
                          isSelected 
                            ? "bg-gradient-to-r from-cyan-500/20 to-emerald-500/20 border-l-4 border-l-cyan-400" 
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                              ...
                              {result.category && (
                                <span className="px-2 py-0.5 text-xs font-medium rounded-md bg-slate-800/50 text-slate-300">
                                  {result.category}
                                </span>
                              )}
                            </div>
              ...
              {results.authors.length > 0 && (
                <div className="border-b border-cyan-400/10">
                  <div className="sticky top-0 z-10 px-4 py-2.5 bg-gradient-to-r from-violet-500/10 to-pink-500/10 border-b border-violet-400/20 backdrop-blur-sm">
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          "cursor-pointer p-4 transition-all border-b border-slate-800/50 last:border-0",
                          isSelected 
                            ? "bg-gradient-to-r from-violet-500/20 to-pink-500/20 border-l-4 border-l-violet-400" 
                            : "hover:bg-slate-900/50"
                        )}
                      >
              ...
              {results.prs.length > 0 && (
                <div>
                  <div className="sticky top-0 z-10 px-4 py-2.5 bg-gradient-to-r from-blue-500/10 to-indigo-500/10 border-b border-blue-400/20 backdrop-blur-sm">
                            ...
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              {result.state && (
                                <span className={cn(
                                  ...
                                  result.state === 'closed' ? "bg-slate-500/20 text-slate-300" :
              ...
              {/* Keyboard hints */}
              {hasResults && (
                <div className="px-4 py-3 bg-slate-900/50 border-t border-cyan-400/10">
                  <div className="flex items-center justify-between text-xs">
                    <div className="flex items-center gap-4">
                      <span className="flex items-center gap-1.5 text-slate-400">
                        <kbd className="px-2 py-1 bg-slate-800 border border-slate-700 rounded shadow-sm font-mono font-semibold text-slate-300">↑</kbd>
                        <kbd className="px-2 py-1 bg-slate-800 border border-slate-700 rounded shadow-sm font-mono font-semibold text-slate-300">↓</kbd>
                      ...
                      <span className="flex items-center gap-1.5 text-slate-400">
                        <kbd className="px-2 py-1 bg-slate-800 border border-slate-700 rounded shadow-sm font-mono font-semibold text-slate-300">Esc</kbd>
                        <span>Close</span>
                      </span>
                    </div>
