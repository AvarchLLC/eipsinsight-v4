...
/**
 * Editorial-activity dashboard for a recurring editor call series (EIP Editing
 * Office Hours, EIPIP, …). All data comes from the getEventDay* analytics
 * procedures, which take a date window; the meeting picker just moves that
 * window onto a specific meeting date. Parameterized by the meeting list so the
 * same dashboard serves any series.
 */

export type MeetingLite = { meeting: number; dateISO: string; timeUTC: string | null };
...
export function EditorialActivityDashboard({
  meetings,
  ...
  meetingNoun?: string;
  meetingNounPlural?: string;
  csvPrefix?: string;
  footer?: React.ReactNode;
}) {
  const pluralNoun = meetingNounPlural ?? `${meetingNoun}s`;
  ...
  useEffect(() => {
    ...
    if (d) setDay(d);
    if (m === "day" && d && meetings.some((x) => x.dateISO === d)) setFilterMode("meeting");
  ...
  const selectedMeeting = useMemo(
    () => (mode === "day" ? meetings.find((m) => m.dateISO === day) ?? null : null),
    [mode, day, meetings],
  );
  const latestPastMeeting = useMemo(() => meetings.find((m) => m.dateISO <= today) ?? meetings[0] ?? null, [today, meetings]);
  const [filterMode, setFilterMode] = useState<"custom" | "meeting">("custom");
  const goCustom = () => { setFilterMode("custom"); setMode("range"); setFrom(monthStart); setTo(today); setCustomOpen(false); setMeetingOpen(false); };
  const goMeeting = () => { setFilterMode("meeting"); setCustomOpen(false); setMeetingOpen(false); if (!selectedMeeting && latestPastMeeting) { setMode("day"); setDay(latestPastMeeting.dateISO); } };
  ...
  const exportPRs = () => {
    downloadCsv(`${csvPrefix}-prs-${winFrom}_${winTo}.csv`, toCsv(prList.map((p) => ({
  ...
  const exportEditors = () => {
    downloadCsv(`${csvPrefix}-editors-${winFrom}_${winTo}.csv`, toCsv(leaderboard.map((e) => ({
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="rounded-xl border border-border bg-card/60 p-3">
        <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
          <div className="inline-flex items-center rounded-lg border border-border bg-muted/50 p-0.5">
            ...
            <button
              ...
              className={cn("inline-flex items-center gap-1.5 rounded-md px-3 py-1 font-medium transition-colors disabled:opacity-40", filterMode === "meeting" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}
            >
              <CalendarClock className="h-3.5 w-3.5" /> {meetingNoun}
            </button>
          </div>
          ...
          {filterMode === "custom" ? (
            ...
            <div className="relative">
              <button
                ...
                title={`Pick a ${meetingNoun}`}
                className={cn(
                  "inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1.5",
                  selectedMeeting ? "border-primary/50 bg-primary/10 text-primary" : "border-border bg-muted/50 text-muted-foreground hover:border-primary/40 hover:text-foreground",
                )}
              >
                <CalendarClock className="h-3.5 w-3.5" />
                {selectedMeeting
                  ? `${meetingNoun} #${selectedMeeting.meeting} · ${prettyDate(selectedMeeting.dateISO)}${selectedMeeting.timeUTC ? ` · ${selectedMeeting.timeUTC} UTC` : ""}`
                  : `Select a ${meetingNoun}`}
                <ChevronDown className={cn("h-3.5 w-3.5 transition-transform", meetingOpen && "rotate-180")} />
              </button>
              {meetingOpen && (
                <>
                  <div className="fixed inset-0 z-10" onClick={() => setMeetingOpen(false)} />
                  <div className="absolute left-0 z-20 mt-1 max-h-80 w-80 overflow-y-auto rounded-lg border border-border bg-card p-1 shadow-lg">
                    <p className="px-2 py-1 text-[10px] uppercase tracking-wide text-muted-foreground">
                      {pluralNoun} ({meetings.length})
                    </p>
                    {meetings.map((m) => {
                      const active = selectedMeeting?.meeting === m.meeting;
                      return (
                        <button
                          key={m.meeting}
                          onClick={() => { setMode("day"); setDay(m.dateISO); setCustomOpen(false); setMeetingOpen(false); }}
                          className={cn(
        ...
        {refreshedAt && (
          <p className="mt-2 text-[11px] text-muted-foreground">
            ...
            {selectedMeeting && (
              <> · <span className="text-foreground">{meetingNoun} #{selectedMeeting.meeting}</span>{selectedMeeting.timeUTC ? ` at ${selectedMeeting.timeUTC} UTC` : ""}</>
            )}
      ...
      {loading && leaderboard.length === 0 ? (
        <div className="rounded-xl border border-border bg-card/60 py-16"><InlineBrandLoader size="md" label="Loading editorial activity…" /></div>
      ) : (
        <>
