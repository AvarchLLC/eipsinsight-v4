...
// Side exits: Stagnant, Withdrawn
const mainFlow = ['Draft', 'Review', 'Last Call', 'Final'];
const sideStatuses = ['Stagnant', 'Withdrawn'];

export function StatusFlowGraph({ data, loading }: StatusFlowGraphProps) {
  if (loading) {
    return (
      <div className="p-6 rounded-xl bg-slate-900/50 border border-slate-700/40">
  ...
  return (
    ...
    >
          ...
          return (
            <React.Fragment key={status}>
              ...
              >
                ...
                <div className="w-full h-1.5 mt-2 bg-slate-800 rounded-full overflow-hidden">
                  <motion.div
                    ...
                    className={cn("h-full rounded-full", color.bg.replace('/20', '/60'))}
                  />
                </div>
              </motion.div>
              ...
              {index < mainFlow.length - 1 && (
                ...
                >
                  <div className="w-8 h-0.5 bg-gradient-to-r from-slate-600 to-slate-700" />
      ...
      {/* Side Statuses */}
      <div className="flex items-center justify-center gap-8 pt-4 border-t border-slate-700/40">
        <span className="text-xs text-slate-500 uppercase tracking-wider">
          Other statuses:
        </span>
        {sideStatuses.map((status, index) => {
          const count = dataMap.get(status) || 0;
          const color = statusColors[status];
