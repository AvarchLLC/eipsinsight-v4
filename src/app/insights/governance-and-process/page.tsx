      ...
      <div className="border-b border-slate-800/50 bg-slate-900/30 backdrop-blur-sm sticky top-0 z-40">
        <div className="container mx-auto px-4 py-4">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-white">Governance & Process Insights</h1>
              <p className="text-sm text-slate-400 mt-1">How reviews, editors, and PR flow shape outcomes.</p>
            </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="rounded-xl border border-slate-700/50 bg-slate-900/40 p-6 backdrop-blur-sm">
              <h3 className="text-lg font-semibold text-white mb-4">PR Lifecycle Funnel</h3>
            ...
            <div className="rounded-xl border border-slate-700/50 bg-slate-900/40 p-6 backdrop-blur-sm">
              <h3 className="text-lg font-semibold text-white mb-4">Current Governance States</h3>
            ...
            <div className="rounded-xl border border-slate-700/50 bg-slate-900/40 backdrop-blur-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-700/50">
                <h3 className="text-lg font-semibold text-white">Time-to-Decision</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-700/50 bg-slate-800/20">
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-400">Repo</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-slate-400">Outcome</th>
                      <th className="px-4 py-3 text-right text-xs font-medium text-slate-400">Median Days</th>
                      <th className="px-4 py-3 text-right text-xs font-medium text-slate-400">Avg Days</th>
                      <th className="px-4 py-3 text-right text-xs font-medium text-slate-400">Count</th>
                    </tr>
                  </thead>
                  <tbody>
                    {ttd.map((r, i) => (
                      <tr key={i} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                        <td className="px-6 py-3 text-white font-medium">{r.repo.toUpperCase()}</td>
                        <td className="px-4 py-3"><span className={cn("text-xs font-medium px-2 py-0.5 rounded-full", r.outcome === "merged" ? "bg-emerald-500/20 text-emerald-300" : "bg-slate-500/20 text-slate-300")}>{r.outcome}</span></td>
                        <td className="px-4 py-3 text-right text-slate-300">{r.medianDays}d</td>
                        <td className="px-4 py-3 text-right text-slate-300">{r.avgDays}d</td>
                        <td className="px-4 py-3 text-right text-slate-400">{r.count.toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {/* Bottleneck Heatmap */}
            <div className="rounded-xl border border-slate-700/50 bg-slate-900/40 p-6 backdrop-blur-sm">
              <h3 className="text-lg font-semibold text-white mb-4">Bottleneck Heatmap (Recent Months)</h3>
              <div className="overflow-x-auto">
                  ...
                  <thead>
                    <tr className="border-b border-slate-700/50">
                      ...
                      {["WAITING_ON_EDITOR", "WAITING_ON_AUTHOR", "STALLED", "MERGED", "CLOSED"].map((s) => (
                        <th key={s} className="px-4 py-2 text-center text-slate-400">{s.replace(/_/g, " ")}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {heatmapByMonth.map((row) => (
                      <tr key={row.month} className="border-b border-slate-800/50">
                        <td className="px-4 py-2 text-white font-medium">{row.month}</td>
