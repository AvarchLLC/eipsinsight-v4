    ...
    <div className="min-h-screen bg-linear-to-br from-slate-950 via-slate-900 to-slate-950">
      <div className="border-b border-slate-800/50 bg-slate-900/30 backdrop-blur-sm sticky top-0 z-40">
        <div className="container mx-auto px-4 py-4">
          ...
          <h1 className="text-2xl font-bold text-white">Upgrade Insights</h1>
          <p className="text-sm text-slate-400 mt-1">What shipped, why, and what changed.</p>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="rounded-xl border border-slate-700/50 bg-slate-900/40 p-6 backdrop-blur-sm">
              <h3 className="text-lg font-semibold text-white mb-6">Upgrade Timeline</h3>
                ...
                <div className="space-y-6">
                  {upgrades.map((upgrade) => (
                    <div key={upgrade.id} className="flex gap-4">
                      ...
                      <div className="rounded-xl border border-slate-700/50 bg-slate-800/30 p-4 flex-1">
                        <div className="flex items-center justify-between gap-4 mb-2">
                          <h4 className="text-base font-semibold text-white">
                            {upgrade.name || upgrade.slug}
                          </h4>
                          <span className="text-xs text-slate-500 shrink-0">{upgrade.createdAt ?? "—"}</span>
                        </div>
                  ...
                  {upgrades.length === 0 && <p className="text-slate-400 text-sm pl-14">No upgrade data available</p>}
                </div>
              </div>
            </div>
            ...
            {compositionData.length > 0 && (
              <div className="rounded-xl border border-slate-700/50 bg-slate-900/40 p-6 backdrop-blur-sm">
                <h3 className="text-lg font-semibold text-white mb-4">Composition Changes Over Time</h3>
                <ChartContainer
